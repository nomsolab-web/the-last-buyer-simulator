...
async function entries(): Promise<Entry[]> {
  ...
  const rows = text.trim().split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line) as Entry)
    .filter((e) => e.path && e.checkedAt && (!SINCE || Date.parse(e.checkedAt) >= SINCE) && (!UNTIL || Date.parse(e.checkedAt) <= UNTIL))
...
async function main() {
  ...
  const states = new Map<string, State>(strategies.map((s) => [s.name, { portfolio: emptyPortfolio(rows[0]?.checkedAt || new Date().toISOString()), trades: [] }]));
  let processed = 0;
  for (const e of rows) {
    let cands: Candidate[];
    try { cands = await loadCandidatesFromReportPath(e.path); } catch { continue; }
    for (const s of strategies) step(s, states.get(s.name) as State, cands, e.checkedAt);
    processed++;
    if (processed % 100 === 0) console.log(`report replay ${processed}/${rows.length}`);
  }
  ...
  const out = { replayMode: 'report_snapshot_exact', generatedAt: new Date().toISOString(), sourceIndex: INDEX_PATH, processedSnapshots: processed, firstSnapshot: rows[0]?.checkedAt ?? null, lastSnapshot: rows[rows.length - 1]?.checkedAt ?? null, assumptions: ['Uses analyzer-produced report snapshots, so discovery/trash/lifecycle/warnings match the saved report input.', 'missing_from_feed and deadCandidate filters are applied during replay.'], strategies: league };
  await fs.mkdir(OUT_DIR, { recursive: true });
  await fs.writeFile(path.join(OUT_DIR, 'replay-report-strategy-league.json'), JSON.stringify(out, null, 2) + '\n', 'utf8');
  ...
  console.log(`wrote ${path.join(OUT_DIR, 'replay-report-strategy-league.json')}`);
}
