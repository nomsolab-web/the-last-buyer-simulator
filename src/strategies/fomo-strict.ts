...
function hasStrongMomentum(candidate: Candidate): boolean {
  return n(candidate.holderMomentum) >= 0.08
    || n(candidate.holderDeltaPct) >= 0.08
    || n(candidate.holderCountChange) >= 5;
}

function hasRecentVolume(candidate: Candidate): boolean {
  return n(candidate.volume5m) > 0 || n(candidate.volume15m) > 0;
}

const strategy: Strategy = {
  name: 'FOMO_STRICT',
  config: { positionSizeSol: 0.1, maxOpenPositions: 4 },
  explainBuy(candidate: Candidate, portfolio: Portfolio) {
    ...
    if ((candidate.liquidityUsd ?? 0) < 50000) reasons.push('liquidity_below_50000');
    if ((candidate.sourceRank ?? Infinity) > 40) reasons.push('source_rank_above_40');
    if ((candidate.trashScore ?? 100) > 60) reasons.push('trash_above_60');
    if (!hasStrongMomentum(candidate)) reasons.push('momentum_below_strict_threshold');
    if (!hasRecentVolume(candidate)) reasons.push('short_volume_missing');
    ...
    if (warningIncludes(candidate, 'unverified liquidity')) reasons.push('unverified_liquidity');
    if (warningIncludes(candidate, 'high bundler')) reasons.push('high_bundler');
    return [...new Set(reasons)];
  },
  shouldBuy(candidate, portfolio) {
    return this.explainBuy!(candidate, portfolio).length === 0;
  },
  shouldSell(position: PaperPosition, candidate: Candidate) {
    ...
    if (position.unrealizedPnlPct >= 50) return { shouldSell: true, reason: 'take_profit_50' };
